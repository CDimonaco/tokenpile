...
Define how tokenpile records token usage from the records agents keep of what the provider reported: the triggers that fire regardless of model behaviour, the readers for each agent, the spool that makes a failed capture recoverable, and how the hooks are installed.
