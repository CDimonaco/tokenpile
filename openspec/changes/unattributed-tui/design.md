...
**3. Group by session alone, with the branch as an attribute.**
...
Adding the branch to the key breaks the same promise from the other direction, which is why it is not in it: assignment operates on a session, so two groups from one session would mean assigning either one silently vacates the other. The branch is therefore reported *about* a group, and collapses to empty when the session's entries disagree — a session that spans a branch switch has no single branch, and reporting one of them would suggest an issue for work only partly done on it.
