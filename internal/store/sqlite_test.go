...
func TestSQLiteStore_LogUsage(t *testing.T) {
	...
	issues, err := s.ListIssues(ctx, domain.IssueFilter{Repo: "owner/repo"})
	require.NoError(t, err)
...
func TestSQLiteStore_LogUsage_SetsTimestamp(t *testing.T) {
	...
	err := s.LogUsage(ctx, domain.UsageEntry{
...
func TestSQLiteStore_ListIssues_MultipleIssues(t *testing.T) {
	...
	for _, num := range []int{1, 2, 3} {
		require.NoError(t, s.LogUsage(ctx, domain.UsageEntry{
			Repo: "o/r", IssueNum: num, Agent: "a", Model: "m",
			TokensIn: 100 * num, TokensOut: 50 * num,
		}))
	}

	issues, err := s.ListIssues(ctx, domain.IssueFilter{Repo: "o/r"})
	require.NoError(t, err)
...
func TestSQLiteStore_ListIssues_FilterByAgent(t *testing.T) {
	...
	require.NoError(t, s.LogUsage(ctx, domain.UsageEntry{Repo: "o/r", IssueNum: 1, Agent: "claude-code", Model: "m", TokensIn: 100}))
	require.NoError(t, s.LogUsage(ctx, domain.UsageEntry{Repo: "o/r", IssueNum: 2, Agent: "opencode", Model: "m", TokensIn: 200}))

	issues, err := s.ListIssues(ctx, domain.IssueFilter{Repo: "o/r", Agent: "claude-code"})
	require.NoError(t, err)
	require.Len(t, issues, 1)
...
func TestSQLiteStore_ListUsageOverTime_DayGranularity(t *testing.T) {
	...
	points, err := s.ListUsageOverTime(ctx, domain.UsageOverTimeFilter{
		Repo:        "o/r",
