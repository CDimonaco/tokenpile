...
- Capture branch: entries store the branch that was checked out when the tokens were spent, and it is never re-derived at listing time — reading the working directory back would report today's branch, which is wrong for exactly the old sessions worth reconciling. The branch-derived issue *suggestion* is the opposite: computed on read via `attribution.InferFromBranch`, never stored, so improving the heuristic needs no migration. Entries captured before the column existed keep an empty branch and suggest nothing rather than being backfilled with fiction.
- Unattributed groups are keyed by session alone, not session and branch. Assignment moves a whole session, so per-branch groups would let assigning one group silently vacate another. A session that spans a branch switch reports no branch and offers no suggestion.
